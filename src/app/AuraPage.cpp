...
AuraPage::AuraPage(DBusClient& client) : m_client(client) {
    m_page = build();
...
GtkWidget* AuraPage::build() {
    ...
    gtk_drop_down_set_selected(m_lightbarDD, 1);
    ...
    gtk_range_set_value(GTK_RANGE(m_speedScale), 2);
...
void AuraPage::updateControlVisibility() {
...
void AuraPage::onModeChanged(GtkDropDown*, GParamSpec*, gpointer self) {
    auto* w = static_cast<AuraPage*>(self);
    w->updateControlVisibility();
    w->applyCurrentSettings();
}

static guint s_speedTimer = 0;
void AuraPage::onSpeedChanged(GtkRange*, gpointer self) {
    if (s_speedTimer) g_source_remove(s_speedTimer);
    s_speedTimer = g_timeout_add(300, [](gpointer data) -> gboolean {
        s_speedTimer = 0;
        static_cast<AuraPage*>(data)->applyCurrentSettings();
        return G_SOURCE_REMOVE;
    }, self);
}

void AuraPage::onBrightnessChanged(GtkRange* range, gpointer self) {
    auto* w = static_cast<AuraPage*>(self);
    w->m_client.setBrightness((uint8_t)gtk_range_get_value(range));
}

void AuraPage::onLightbarModeChanged(GtkDropDown* dd, GParamSpec*, gpointer self) {
    auto* w = static_cast<AuraPage*>(self);
    w->m_client.setLightbarMode((uint8_t)gtk_drop_down_get_selected(dd));
}

static guint s_colorTimer = 0;
void AuraPage::onZoneColorSet(GtkColorDialogButton*, GParamSpec*, gpointer self) {
...
void AuraPage::onBreatheColorSet(GtkColorDialogButton*, GParamSpec*, gpointer self) {
    if (s_colorTimer) g_source_remove(s_colorTimer);
    s_colorTimer = g_timeout_add(300, [](gpointer data) -> gboolean {
        s_colorTimer = 0;
        static_cast<AuraPage*>(data)->applyCurrentSettings();
...
void AuraPage::applyCurrentSettings() {
...
void AuraPage::syncMode(uint8_t mode) {
...
void AuraPage::syncBrightness(uint8_t level) {
